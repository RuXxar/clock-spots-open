...
import type { BoardSlots, Clue, JobId, PositionId, Puzzle, TagId, TetherMarker } from './types';
...
export function evaluateClue(clue: Clue, board: BoardSlots, puzzle: Puzzle): boolean {
  switch (clue.kind) {
    ...
    case 'group-count': {
      const count = positionsInGroup(clue.group).filter((position) => jobTagAt(board, position, clue.tag)).length;
...
export function clueText(clue: Clue): string {
  switch (clue.kind) {
    case 'self-tag':
      return clue.positive ? `I am ${TAG_LABELS[clue.tag].noun}` : `I am not ${TAG_LABELS[clue.tag].noun}`;
    case 'relative-tag':
...
function evaluateMechanic(mechanic: Extract<Clue, { kind: 'mechanic' }>['mechanic'], board: BoardSlots, puzzle: Puzzle): boolean {
  switch (mechanic) {
    case 'boss-aggro':
      return puzzle.markers.aggroPosition ? JOBS[jobAt(board, puzzle.markers.aggroPosition)].role === 'tank' : true;
    case 'vuln-dodge':
    ...
    case 'healing':
      return puzzle.markers.lowHpJobs.every((job) => {
        ...
        return neighborPositions(position).some((neighbor) => JOBS[jobAt(board, neighbor)].role === 'healer');
      });
    case 'hello-world':

...
export function TutorialModal({ open, step, onStep, onClose }: TutorialModalProps) {
  ...
  return (
    <div className="tutorial-popover" role="dialog" aria-modal="true" aria-label="Tutorial">
      <button className="icon-button close" type="button" onClick={onClose} aria-label="Close tutorial">
        <X size={16} />
      </button>
      <p>{STEPS[current]}</p>
      <div className="tutorial-footer">
        ...
        <div>
          ...
          <button type="button" onClick={current === STEPS.length - 1 ? onClose : () => onStep(current + 1)}>
